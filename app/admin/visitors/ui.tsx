...
export default function VisitorsManager({ email }: { email: string }) {
  ...
  function beginNickname(v: Visitor) {
    setEditing(v.visitor_id);
  ...
  async function saveNickname(visitorId: string) {
    ...
    if (!res.ok) {
      setMessage(json.error || 'Nickname could not be saved.');
      setBusy(false);
      return;
    }
    ...
    setVisitors((current) =>
      current.map((v) =>
        v.visitor_id === visitorId
          ? { ...v, nickname: json.visitor.nickname }
          : v,
      ),
    );
  ...
  async function toggleTimeline(visitorId: string) {
    ...
    if (timeline[visitorId]) {
      return;
    }

    setLoadingTimeline(visitorId);
  ...
  const eventLabel = (event: string) =>
    event.replace(/^project_view:/, 'Viewed ').replaceAll('_', ' ');
    ...
    <main className="admin-shell">
      ...
      <header className="admin-topbar">
        <a href="/" className="admin-logo">
          <span>&lt;/&gt;</span> MYLES<span className="dot">.</span>
        </a>
      ...
      <div className="admin-layout">
        <aside className="admin-sidebar">
          ...
          <a className="admin-side-link active" href="/admin/visitors">
        ...
        <section className="admin-main">
          <div className="admin-heading">
            <div>
              ...
              <p>
                Every tracked browser gets a private record. Give people your
                own nickname for quick reference.
              </p>
            </div>
          ...
          <div className="admin-panel visitor-directory-panel">
            ...
            <div className="visitor-search-wrap">
              <Search size={16} />
              ...
              <input
                ...
                placeholder="Search nickname, location, device, page or IP..."
              />
            </div>
              ...
              {filtered.map((v) => (
                ...
                >
                  <div className="visitor-profile-head">
                    <div>
                      <span className="eyebrow">
                        {v.nickname ? 'CUSTOM NAME' : 'UNNAMED VISITOR'}
                      </span>

                      <h3>{v.nickname || 'Unnamed visitor'}</h3>
                    ...
                    <div className="visitor-card-actions">
                      ...
                      <button
                        className="admin-secondary"
                        onClick={() => toggleTimeline(v.visitor_id)}
                      >
                        {expanded === v.visitor_id ? (
                          <ChevronUp size={14} />
                        ) : (
                          <ChevronDown size={14} />
                        )}
                  ...
                  <div className="visitor-tech-strip">
                    <span>
                      <Monitor size={13} /> {v.device} · {v.os} ·{' '}
                      {v.browser}
                    </span>

                    <span>
                      <Activity size={13} /> {v.pageview_count} views ·{' '}
                      {v.interaction_count} interactions
                    </span>
                  </div>

                  {editing === v.visitor_id && (
                    <div className="visitor-nickname-editor">
                      ...
                      <button
                        ...
                        disabled={busy}
                        onClick={() => saveNickname(v.visitor_id)}
                      >
                        <Save size={15} /> Save
                      </button>
                  ...
                  <div className="visitor-profile-grid">
                    <div>
                      ...
                      <b>{new Date(v.first_seen).toLocaleString()}</b>
                    </div>

                    <div>
                      <span>LAST SEEN</span>
                      <b>{new Date(v.last_seen).toLocaleString()}</b>
                    </div>
                    ...
                    <div>
                      ...
                      <b>
                        {revealed.has(v.visitor_id)
                          ? v.ip_full || v.ip_masked || 'Unavailable'
                          : v.ip_masked || 'Unavailable'}
                      </b>

                      {v.ip_full && (
                        <button
                          type="button"
                          onClick={() => toggleIp(v.visitor_id)}
                        >
                          {revealed.has(v.visitor_id) ? (
                            <>
                              <EyeOff size={12} /> Hide
                  ...
                  {expanded === v.visitor_id && (
                    <div className="visitor-timeline">
                      <div className="timeline-heading">
                        ...
                        {loadingTimeline === v.visitor_id && (
                          <span>loading...</span>
                        )}
                      </div>

                      {(timeline[v.visitor_id] || []).map((e) => (
                        <div className="timeline-row" key={e.id}>
                          ...
                          <div className="timeline-copy">
                            ...
                            <span>
                              {e.page} · {e.device} · {e.browser}
                              {e.referrer ? ` · from ${e.referrer}` : ''}
                            </span>
                          </div>
                      ...
                      {!loadingTimeline &&
                        timeline[v.visitor_id] &&
                        !timeline[v.visitor_id].length && (
                          <p className="admin-empty">
                            No activity found for this visitor.
                          </p>
                        )}
                    </div>
                  )}
